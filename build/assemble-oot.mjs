...
/* Assemble Ocarina of Time (game 3) into an app-ready bundle knowledge/oot/app-data.json, shaped exactly like
   ...
   (static identity), derives STATUS_RUNES + wires the CHAMPIONS (Spiritual Stones & Medallions) to the steps
   that grant them, and folds in OPTIONAL depth overlays (idempotent, degrade if absent):
     items-songs.json   → RUNES (the full Items & Songs reference; Guide→Items)
     bestiary.json      → BESTIARY {enemies, basics} (Guide→Enemies; combat primer + boss guides)
     great-fairies.json → GREAT_FAIRIES (Guide→Fairies; the magic-upgrade fountains)
     side-quests.json   → SIDE_QUESTS (Guide→Quests; stable sq_<slug> ids)
   guideSegs rebuilds from which datasets populate. Every other dataset OoT lacks (shrines/cooking/maps/armor/
   towers/koroks/economy) defaults empty and degrades gracefully — TotK v1 was the canary for this. All edits go
   to knowledge/oot/* sources, never the built file. */
