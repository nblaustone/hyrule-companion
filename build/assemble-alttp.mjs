#!/usr/bin/env node
/* Assemble A Link to the Past (game 5, SNES) into an app-ready bundle knowledge/alttp/app-data.json, shaped
   exactly like the BotW/TotK/OoT/MM data the components expect. Reads the hand-authored walkthrough.json (REGIONS)
   + globals.json (static identity), derives STATUS_RUNES + wires the CHAMPIONS (the 3 Pendants + 7 Crystals) to
   the dungeon-reward steps that grant them, and folds in OPTIONAL depth overlays (idempotent, degrade if absent):
     ...
     great-fairies.json → GREAT_FAIRIES (Guide→Fairies; the Great Fairy heart/magic refills)
     side-quests.json   → SIDE_QUESTS (Guide→Quests; bottles, heart pieces, minigames, stable sq_<slug> ids)
     compendium.json    → COMPENDIUM (Items tab catalog)
   guideSegs rebuilds from which datasets populate. Every dataset ALttP lacks (shrines/cooking/maps/armor/towers/
   koroks/economy) defaults empty and degrades gracefully — TotK/OoT/MM v1 were the canaries. All edits go to
   knowledge/alttp/* sources, never the built file. Mirrors build/assemble-mm.mjs. */
import fs from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
const O = join(dirname(fileURLToPath(import.meta.url)), "..", "knowledge", "alttp");
...
const STATUS_PICKS = [
  ["Master Sword", "sword"], ["Pegasus Boots", "stasis"], ["Hookshot", "magnesis"], ["Moon Pearl", "orb"],
];
...
const out = {
  id: "alttp", label: "A Link to the Past", short: "ALttP",
  REGIONS,
  SHRINES: [],
  ARMOR: { sets: [] },
  BESTIARY,
...
console.log("ALttP chapters:", REGIONS.length, "| steps:", steps);
